"""Configuration for the single-node GraphPoint-GC model."""
...
class ModelConfig:
    ...
    # current: expose only the latest shared CLS tokens; all: expose CLS tokens from every history step.
    ...
    def __post_init__(self) -> None:
        if self.policy_name != "graphpoint_gc":
            raise ValueError(
                f"Expected policy_name='graphpoint_gc', got {self.policy_name!r}"
            )
