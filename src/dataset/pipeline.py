...
class GraphVLAPipeline:
    ...
    def run(self):
        ...
        self._print_save_plan(episode_indices)
    ...
    def _process_episode(
        ...
        if not need_node_track and not need_depth:
            return
    ...
    def _build_node_points_track(
        ...
        if not nodes:
            return np.empty((len(frames), 0, self.config.points_per_node, 3), dtype=np.float32)
        ...
        return np.stack(frame_tracks, axis=0).astype(np.float32)
    ...
    def _build_depths_rel(
        ...
        for frame_index, frame in enumerate(frames):
            ...
            masks.append(self.binary_segmenter.predict(frame, mode="foreground"))

        calibrator = RawDepthShiftCalibrator(depths[0], masks[0])
        depths_rel = [depths[0]]
        for depth, mask in zip(depths[1:], masks[1:]):
            try:
    ...
    def _print_save_plan(self, episode_indices: Sequence[int]):
        cs.rule()
        cs.print("[bold cyan]Pipeline outputs[/bold cyan]")
        cs.print(f"taskstructures jsonl: {self.taskstructures_jsonl_path}")
        for episode_index in episode_indices:
            cs.print(f"parquet: {self._episode_parquet_path(episode_index)}")
        cs.print("parquet fields: node_points_track, depths_rel")
        if self.config.debug:
