...
class OfflinePipeline:
    """Add PointTracker camera-XYZ tracks to a LIBERO dataset."""
    ...
    def _process_episode(
        ...
        if "node_points_xyz_track" in df.columns:
            return
        ...
        node_xyz, _, _ = self._build_node_points_xyz(
            tracks, self._read_metric_depths(df, horizontal_flip=frame_flipped), intrinsic,
        )
        df["node_points_xyz_track"] = [points.tolist() for points in node_xyz]
    ...
    def _write_episode_parquet(self, df: pd.DataFrame, parquet_path: Path) -> None:
        ...
        target_types = {
            "node_points_xyz_track": pa.list_(pa.list_(pa.list_(pa.float32()))),
        }
    ...
    def _print_save_plan(self):
        ...
        if self.config.debug:
            ...
            cs.print("parquet field: node_points_xyz_track (add only; existing values are kept)")
        if self.config.debug:
    ...
    def _ensure_point_tracker(self):
        if self.point_tracker is None:
            self.point_tracker = PointTracker(device=self.config.tracker_device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug-dir", default="/data0/luokang/research/GraphVLA/__tmp__/pipeline_track")
    args = parser.parse_args()
    config = PipelineConfig(
        dataset_dir=args.dataset_dir,
        episode_selector={},
        tracker_device=args.device,
        task_analyzer_api_key=os.environ.get("TASK_ANALYZER_API_KEY"),
        debug=args.debug,
