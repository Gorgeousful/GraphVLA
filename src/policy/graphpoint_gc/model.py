"""Single-node flow-matching model for GraphPoint-GC."""
...
from src.policy.graphpoint_gc.encoder import CollapsedEncoder
from src.policy.graphpoint_gc.flow_matching import (
    make_scheduler,
    sample_time,
    training_path,
)
from src.policy.graphpoint_gc.temporal import (
    AdaptiveLayerNorm,
    AdaRMSNorm,
    RotaryFlowBlock,
)
...
class GraphFlowModel(GradientCheckpointingMixin, nn.Module):
    ...
    def __init__(
        ...
        self.progress_head = ConditionedProgressHead(
            relation_dim=hidden_dim * cls_token_num,
    ...
    def _memory_mask(self, batch: dict[str, Any]) -> torch.Tensor | None:
        # Shared summary tokens remain valid when an individual role is absent.
        return None
