...
class FoundationPose:
  ...
  def make_rotation_grid(self, min_n_views=40, inplane_step=60):
    ...
    if mycpp is None or not hasattr(mycpp, "cluster_poses"):
      logging.warning(
        "mycpp extension is not available; skipping pose clustering. "
        "Build it via: cd FoundationPose/mycpp && mkdir -p build && cd build && cmake .. && make -j"
      )
    else:
