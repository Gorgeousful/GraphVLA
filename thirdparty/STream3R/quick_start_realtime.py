...
def save_predictions(predictions, images_shape_hw, image_names, result_dir):
    os.makedirs(result_dir, exist_ok=True)
    extrinsic, intrinsic = pose_encoding_to_extri_intri(predictions["pose_enc"], images_shape_hw)
    ...
    save_cameras_json(
        ...
        to_numpy(intrinsic),
        image_names,
        os.path.join(result_dir, "cameras.json"),
    )
...
def run_stream_test(model, images, image_names, mode, result_dir, window_size=5):
    ...
    save_predictions(predictions, images.shape[-2:], image_names, result_dir)
    session.clear()
    print(f"Saved {mode} results to {result_dir}")
